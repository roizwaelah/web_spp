  ...
  return (
    <div className="table-wrap">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200 text-[14px] xl:text-[0.8rem]">
          ...
          <tbody className={striped ? "divide-y divide-slate-200" : "divide-y divide-slate-300"}>
            {safeRows.length === 0 ? (
              <tr>
              ...
              >
                {safeColumns.map((column) => (
                  <td
                    key={column.key}
                    className={`px-3 py-2.5 align-top text-black xl:px-3 xl:py-2 ${column.cellClassName || ""}`.trim()}
                  >
