...
export default function FinancePostsPage() {
  ...
  const targetLabel = useMemo(() => form.applies_to === 'class' ? 'kelas' : 'siswa', [form.applies_to])

  return (
    <Layout title="Manajemen Pos Keuangan" subtitle="Atur jenis pembayaran SPP, uang gedung, seragam, dan tagihan khusus per kelas atau per siswa.">
      <div className="page-grid">
        <div className="card p-6">
          <h3 className="section-title">{form.id ? 'Edit pos keuangan' : 'Tambah pos keuangan'}</h3>
          <form className="mt-4 space-y-4" onSubmit={submit}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <select className="input" value={form.applies_to} onChange={(e) => setForm({ ...form, applies_to: e.target.value, class_id: '', student_id: '' })}>
            ...
            <div className="flex gap-3">
              <button className="btn-primary flex-1">{form.id ? 'Update pos' : 'Simpan pos'}</button>
        ...
        <Table
          columns={[
            ...
            { key: 'amount', title: 'Tarif', render: (row) => formatCurrency(row.amount) },
            { key: 'billing_type', title: 'Jenis' },
            { key: 'scope', title: 'Target', render: (row) => row.applies_to === 'class' ? row.class_name : row.student_name },
