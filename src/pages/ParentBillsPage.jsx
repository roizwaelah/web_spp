...
export default function ParentBillsPage() {
  ...
  const pay = async (billId, channel) => {
    const { data } = await fetchRoute('parent/payments', { method: 'POST', data: { bill_id: billId, payment_channel: channel } })
  ...
  const uploadProof = async (billId) => {
    ...
    const res = await fetchRoute('parent/payment-proofs', { method: 'POST', data, headers: { 'Content-Type': 'multipart/form-data' } })
  ...
  return (
    <Layout title="Tagihan Saya" subtitle="Lakukan pembayaran otomatis atau unggah bukti transfer manual untuk diverifikasi admin.">
        ...
        columns={[
          ...
          { key: 'proof_status', title: 'Bukti Bayar', render: (row) => row.proof_status ? <span className={row.proof_status === 'approved' ? 'badge-green' : row.proof_status === 'rejected' ? 'badge-red' : 'badge-amber'}>{row.proof_status}</span> : <span className="badge-slate">-</span> },
          {
            ...
            render: (row) => row.status === 'paid' ? (
              <a className="btn-secondary" href={fileUrl('parent/receipt', { bill_id: row.id })} target="_blank" rel="noreferrer">Cetak bukti</a>
            ) : (
              <div className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  {['Transfer Bank', 'QRIS', 'Virtual Account', 'E-Wallet'].map((channel) => (
                    <button key={channel} className="btn-secondary text-xs" onClick={() => pay(row.id, channel)}>{channel}</button>
                  ))}
                </div>
                <div className="rounded-2xl border border-slate-200 p-3">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Upload bukti transfer manual</p>
                  <div className="flex flex-col gap-2 md:flex-row">
                    <input type="file" accept=".jpg,.jpeg,.png,.pdf" className="input" onChange={(e) => setFileMap((prev) => ({ ...prev, [row.id]: e.target.files?.[0] || null }))} />
                    <button className="btn-primary" onClick={() => uploadProof(row.id)}>Kirim</button>
                  </div>
                </div>
              </div>
            ),
