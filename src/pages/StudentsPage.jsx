...
export default function StudentsPage() {
  ...
  const importStudents = async () => {
    ...
    await fetchRoute('admin/students/import', { method: 'POST', data, headers: { 'Content-Type': 'multipart/form-data' } })
    setMessage('Impor data berhasil')
    load()
  }

  const filtered = useMemo(() => students.filter((item) => `${item.name} ${item.nis} ${item.parent_name} ${item.class_name}`.toLowerCase().includes(filter.toLowerCase())), [students, filter])

  return (
    <Layout title="Manajemen Data Siswa" subtitle="CRUD lengkap siswa, edit/hapus data, relasi kelas dan tahun ajaran, serta impor Excel/CSV.">
      <div className="page-grid">
        <div className="card p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select className="input" value={form.academic_year_id} onChange={(e) => setForm({ ...form, academic_year_id: e.target.value })}>
                  ...
                  {meta.years.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
                </select>
              </div>
            </div>
          ...
          <div className="mt-6 rounded-3xl bg-slate-50 p-4">
            <div className="mb-3 flex items-center gap-2 font-semibold text-slate-800"><Upload size={18} /> Impor Excel / CSV</div>
            <div className="flex flex-col gap-3 md:flex-row">
              <input type="file" className="input" accept=".csv,.xlsx,.xls" onChange={(e) => setFile(e.target.files?.[0] || null)} />
              <button type="button" className="btn-secondary" onClick={importStudents}>Impor</button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="card p-6">
            <input className="input" placeholder="Cari nama / NIS / orang tua / kelas" value={filter} onChange={(e) => setFilter(e.target.value)} />
          </div>
          ...
          <Table
            columns={[
              ...
              { key: 'status', title: 'Status', render: (row) => <span className={row.status === 'active' ? 'badge-green' : 'badge-amber'}>{row.status}</span> },
